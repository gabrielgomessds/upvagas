...
@section('title', 'Aplicações para a Vaga')
  ...
  <div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
      <div class="card">
        ...
        <h5 class="card-header">Aplicações para - <b class="text-primary">{{ $vacancy->title }}</b>
        @if(isset($company))
        ...
        <div class="table-responsive text-nowrap">
          ...
          <table class="table">
            ...
            <tbody class="table-border-bottom-0">
              ...
              <tr>
                <td>{{ mb_strimwidth($app->user->name,0,40,'...') }}</td>
                <td>{{ $app->situation == 'sent' ? 'Enviado' :  ($app->situation == 'accepted' ? 'Aceito' :  'Recusado') }}</td>
                <td><a href="{{ '/admin/vaga/aplicacoes/curriculo/'.base64_encode($app->id) }}"><button class="btn btn-sm btn-primary">Ver <i class='bx bx-file'></i></button> </td>
                <td><a href="{{ url('/admin/vaga/'.$app->id.'/editar') }}">
                    <i class='bx bxs-pencil'></i></a>
                  <a href="#" class="button-modal-list" data-title-modal="Tem certeza que deseja excluir essa vaga?" data-url-modal="{{ url('/admin/vaga/'.$app->id.'/excluir') }}" data-bs-toggle="modal" data-bs-target="#modalCenter"><i class='bx bxs-trash'></i></a>
